...
def retrieve_cimis_station_info(verbose=False):
    ...
    try:
        data = urllib.request.urlopen(station_url).read()
        #print("Retrieved " + str(len(data)) + " characters" )
        content = json.loads(data.decode('utf-8'))
        # content = json.loads(urllib.request.urlopen(station_url).read().decode('utf-8'))
...
def retrieve_cimis_data(url, target):
    try:
        ...
        # -- testing to see if JSON file is valid (temp) 
